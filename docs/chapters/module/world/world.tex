...
\begin{example}{Create an extra the\_nether dimension}
    \cmd{/world create my\_nether minecraft:the\_nether}
\end{example}

\begin{example}{Delete the extra dimension}
    \cmd{/world delete fuji:my\_nether}
\end{example}

\begin{example}{Reset the extra dimension with random seed}
    \cmd{/world reset fuji:my\_nether}
\end{example}

\begin{example}{Specify a seed for an extra dimension.}
    \cmd{/world create my\_nether --seed 1234567890 minecraft:the\_nether}\\
    \cmd{/world reset --useTheSameSeed true fuji:my\_nether}
\end{example}

\begin{tips}{Make a resource-world that automatically reset every day}
    You can use~\nameref{ch:command_scheduler} module to execute \cmd{/world reset} command automatically.
\end{tips}