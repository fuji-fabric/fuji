...
public class FabricApiHelper {
    ...
    public static void fireOnWorldLoadEvent(@NotNull MinecraftServer server, @NotNull ServerLevel serverWorld) throws Exception {
        ...
        FabricApiHelper.fireEvent("net.fabricmc.fabric.api.event.lifecycle.v1.ServerWorldEvents", "LOAD", "onWorldLoad", eventParameters, eventArguments);
        FabricApiHelper.fireEvent("net.fabricmc.fabric.api.event.lifecycle.v1.ServerLevelEvents", "LOAD", "onLevelLoad", eventParameters, eventArguments);
    }
    ...
    public static void fireOnWorldUnloadEvent(@NotNull MinecraftServer server, @NotNull ServerLevel serverWorld) throws Exception {
        List<Class<?>> eventParameters = List.of(MinecraftServer.class, ServerLevel.class);
        List<Object> eventArguments = List.of(server, serverWorld);
        FabricApiHelper.fireEvent("net.fabricmc.fabric.api.event.lifecycle.v1.ServerWorldEvents", "UNLOAD", "onWorldUnload", eventParameters, eventArguments);
        FabricApiHelper.fireEvent("net.fabricmc.fabric.api.event.lifecycle.v1.ServerLevelEvents", "UNLOAD", "onLevelUnload", eventParameters, eventArguments);
    }
    ...
    private static void fireEvent(@NotNull String eventDeclaringClassName, @NotNull String eventDeclaringFieldName, @NotNull String eventFireMethodName, @NotNull List<Class<?>> eventFireMethodParameterList, @NotNull List<Object> eventFireMethodArgumentList) throws Exception {
        ...
        try {
            ...
            eventFireMethod.invoke(eventInvokerInstance, eventFireMethodArgumentArray);
                 ...
                 IllegalArgumentException | NoSuchFieldException error) {
            LogUtil.debug("""
                Failed to fire the specified event, now ignoring it.
                ◉ Event Declaring Class Name: {}
                ◉ Event Declaring Field Name: {}
                ◉ Event Declaring Method Name: {}
                ◉ Event Fire Method Parameter List: {}
                ◉ Event Fire Method Argument List: {}
                ◉ Error: {}
                """, eventDeclaringClassName, eventDeclaringFieldName, eventFireMethodName, eventFireMethodParameterList, eventFireMethodArgumentList, error);
        }
    }
}
