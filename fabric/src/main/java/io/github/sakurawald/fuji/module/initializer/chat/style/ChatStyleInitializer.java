...
public class ChatStyleInitializer extends ModuleInitializer {
    ...
    private static final NodeParser CHAT_STYLE_PARSER = TextHelper.Parsers.STYLE_ONLY_PARSER;
    ...
    private static int setPerPlayerFormat(@CommandSource @CommandTarget ServerPlayerEntity player, GreedyString format) {
        ...
        /* Feedback. */
        newFormat = TextHelper.Mapper.getLanguageValueByKey(player, "chat.format.set");
        newFormat = newFormat.replace("%s", stripedFormat);
        newFormat = newFormat.replace("%message%", TextHelper.Mapper.getLanguageValueByKey(player, "chat.format.show"));
